...
int main(int argc, char** argv) {
    if (argc != 6) {
        std::cout << argc << argv[0]
                  << " data_file IndexKey SearchKey K saving_graph"
                  << std::endl;
        exit(-1);
    }
    ...
    data_load = efanna2e::data_align(
        data_load, points_num, dim); // one must align the data before build
    ...
    for (unsigned i = 0; i < points_num; i++) {
        ...
        cnt++;
        if (cnt % 100000 == 0) {
            std::cout << cnt / 100000 << " / " << points_num / 100000
                      << " complete\n";
        }
    }
