...
namespace efanna2e {
    ...
    struct Candidate {
        size_t row_id;
        float distance;
        Candidate(const size_t row_id, const float distance)
            : row_id(row_id), distance(distance) {}
    ...
    public:
        explicit IndexKDtree(const size_t dimension, const size_t n, Metric m,
                             Index* initializer);
    ...
    protected:
        ...
        enum {
             ...
             *
             * When creating random trees, the dimension on which to subdivide
             * is selected at random from among the top RAND_DIM dimensions with
             * the highest variance.  A value of 5 works well.
             */
            RAND_DIM = 5
        };
    ...
    private:
        void meanSplit(std::mt19937& rng, unsigned* indices, unsigned count,
                       unsigned& index, unsigned& cutdim, float& cutval);
        void planeSplit(unsigned* indices, unsigned count, unsigned cutdim,
                        float cutval, unsigned& lim1, unsigned& lim2);
        ...
        void DFSbuild(Node* node, std::mt19937& rng, unsigned* indices,
