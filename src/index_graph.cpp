...
#pragma omp parallel for default(shared) schedule(dynamic, 100)
        for (unsigned n = 0; n < nd_; n++) {
            graph_[n].join([&](unsigned i, unsigned j) {
                if (i != j) {
                    float dist =
                        distance_->compare(data_ + i * dimension_,
...
#pragma omp parallel for
        for (unsigned n = 0; n < nd_; ++n) {
            ...
            for (unsigned l = 0; l < nnhd.M; ++l) {
                auto& nn = nnhd.pool[l];
                auto& nhood_o =
                    graph_[nn.id]; // nn on the other side of the edge
...
#pragma omp parallel for
        for (unsigned i = 0; i < c.size(); i++) {
            ...
            for (unsigned j = 0; j < N; j++) {
                float dist =
                    distance_->compare(data_ + c[i] * dimension_,
                                       data_ + j * dimension_, dimension_);
                tmp.push_back(Neighbor(j, dist, true));
            }
            std::partial_sort(tmp.begin(), tmp.begin() + _CONTROL_NUM,
                              tmp.end());
    ...
    void
    IndexGraph::eval_recall(std::vector<unsigned>& ctrl_points,
...
#pragma omp parallel for
    ...
    void IndexGraph::RefineGraph(const float* data,
            ...
            if (retset[k].flag) {
                ...
                for (unsigned m = 0; m < final_graph_[n].size(); ++m) {
                    ...
                    float dist = distance_->compare(
                        query, data_ + dimension_ * id, (unsigned)dimension_);
    ...
    void IndexGraph::parallel_graph_insert(unsigned id, Neighbor nn,
                                           LockGraph& g, size_t K) {
...
#pragma omp for
            for (unsigned i = 0; i < n_new; i++) {
                ...
                get_neighbor_to_add(data + i * dim, parameters, graph_tmp, rng,
                                    res, n_new);

                for (unsigned j = 0; j < K; j++) {
                    parallel_graph_insert(i + (unsigned)nd_, res[j], graph_tmp,
                                          K);
                    parallel_graph_insert(
                        res[j].id,
                        Neighbor(i + (unsigned)nd_, res[j].distance, true),
                        graph_tmp, K);
                }
            }
        };
        ...
        while (k < (int)L) {
            ...
            if (retset[k].flag) {
                ...
                for (unsigned m = 0; m < g[n].pool.size(); ++m) {
                    ...
                    flags[id] = 1;
                    float dist = distance_->compare(
                        point, data_ + dimension_ * id, (unsigned)dimension_);
                    if (dist >= retset[L - 1].distance)
                        continue;
                    Neighbor nn(id, dist, true);
                    int r = InsertIntoPool(retset.data(), L, nn);
