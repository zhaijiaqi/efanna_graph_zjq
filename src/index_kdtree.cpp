...
#define _CONTROL_NUM 100
    IndexKDtree::IndexKDtree(const size_t dimension, const size_t n, Metric m,
                             Index* initializer)
    ...
    void IndexKDtree::meanSplit(std::mt19937& rng, unsigned* indices,
    ...
    void IndexKDtree::planeSplit(unsigned* indices, unsigned count,
                                 unsigned cutdim, float cutval, unsigned& lim1,
                                 unsigned& lim2) {
    ...
    int IndexKDtree::selectDivision(std::mt19937& rng, float* v) {
        ...
        for (size_t i = 0; i < dimension_; ++i) {
            if ((num < RAND_DIM) || (v[i] > v[topind[num - 1]])) {
                ...
                // sort the varience in decrease order
                int j = num - 1;
    ...
    void IndexKDtree::DFSbuild(Node* node, std::mt19937& rng, unsigned* indices,
    ...
    void IndexKDtree::DFStest(unsigned level, unsigned dim, Node* node) {
        if (node->Lchild != NULL) {
            ...
            // if(level > 15)
            std::cout << "dim: " << node->DivDim << "--cutval: " << node->DivVal
                      << "--S: " << node->StartIdx << "--E: " << node->EndIdx
                      << " TREE: " << node->treeid << std::endl;
            if (node->Lchild->Lchild == NULL) {
                std::vector<unsigned>& tmp = LeafLists[node->treeid];
                for (unsigned i = node->Rchild->StartIdx;
                     i < node->Rchild->EndIdx; i++) {
                    const float* tmpfea =
                        data_ + tmp[i] * dimension_ + node->DivDim;
                    std::cout << *tmpfea << " ";
                }
                std::cout << std::endl;
            }
    ...
    void IndexKDtree::getMergeLevelNodeList(Node* node, size_t treeid,
                                            int deepth) {
            ...
            for (; start < end; start++) {
                ...
                for (size_t i = leaf->StartIdx; i < leaf->EndIdx; i++) {
                    ...
                    float dist = distance_->compare(
                        data_ + tmpfea * dimension_,
                        data_ + feature_id * dimension_, dimension_);

                    {
                        LockGuard guard(graph_[tmpfea].lock);
                        if (knn_graph[tmpfea].size() < K ||
                            dist < knn_graph[tmpfea].begin()->distance) {
                            Candidate c1(feature_id, dist);
                            knn_graph[tmpfea].insert(c1);
                            if (knn_graph[tmpfea].size() > K)
                                knn_graph[tmpfea].erase(
                                    knn_graph[tmpfea].begin());
                        }
                    }

                    {
                        ...
                        if (knn_graph[feature_id].size() < K ||
                            ...
                            if (knn_graph[feature_id].size() > K)
                                knn_graph[feature_id].erase(
                                    knn_graph[feature_id].begin());
                        }
                    }
                }
            }
    ...
    void IndexKDtree::Build(size_t n, const float* data,
...
#pragma omp parallel for
            for (unsigned i = 0; i < ActiveSet.size(); i++) {
                ...
                meanSplit(rng, &myids[0] + node->StartIdx,
                          node->EndIdx - node->StartIdx, mid, cutdim, cutval);
                ...
                omp_set_lock(&rootlock);
                if (mid > K)
                    NewSet.push_back(nodeL);
                if (nodeR->EndIdx - nodeR->StartIdx > K)
                    NewSet.push_back(nodeR);
                omp_unset_lock(&rootlock);
            }
...
#pragma omp parallel for
        for (unsigned i = 0; i < ActiveSet.size(); i++) {
            ...
            std::mt19937 rng(seed ^ omp_get_thread_num());
            std::vector<unsigned>& myids = LeafLists[node->treeid];
            DFSbuild(node, rng, &myids[0] + node->StartIdx,
                     node->EndIdx - node->StartIdx, node->StartIdx);
        }
        ...
        if (error_flag) {
            std::cout << "merge level deeper than tree, max merge deepth is "
                      << max_deepth - 1 << std::endl;
        }
...
#pragma omp parallel for
        ...
        for (unsigned i = 0; i < nd_; i++) {
            ...
            if (tmp.size() < K) {
                // std::cout << "node "<< i << " only has "<< tmp.size() <<"
                // neighbors!" << std::endl;
                result.clear();
    ...
    void IndexKDtree::Search(const float* query, const float* x, size_t k,
